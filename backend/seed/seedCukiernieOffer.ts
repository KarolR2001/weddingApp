...
async function generateCompanyName(categoryName: string): Promise<string> {
    const keywords = ["Słodkość", "Tort", "Wypiek", "Smak", "Delikatność", "Pyszność", "Ciasto", "Krem", "Dekoracja", "Elegancja", "Uroda", "Harmonia", "Kreatywność", "Pasja", "Styl", "Słodycze", "Deser", "Cukier", "Radość", "Ślub", "Wesele", "Słodki Stół", "Ciastko", "Beza", "Makaronik", "Czekolada", "Owoc", "Tradycja", "Nowoczesność", "Aromat", "Smakołyk", "Finezja"];
    const baseNames = ["Cukiernia", "Pracownia", "Słodkości", "Piekarnia"];
    const additionalWords = ["Ślubna", "Weselna", "Kreatywna", "Elegancka"];
    ...
    const prompt = `
      ...
      2. Poprawna gramatyka i odmiana (np. "Cukiernia Ślubna" a nie "Cukiernia Ślub", "Pracownia Słodkości" a nie "Pracownia Słodkość")
      3. Zakazane elementy:
         - Cyfry w jakiejkolwiek formie (np. "Cukiernia 2", "Piekarnia Trzy")
         - Znaki specjalne poza myślnikami
         - Skróty i numery porządkowe (II, III, 1st)
      4. Preferowane są polskie nazwy, używające słów takich jak "${baseNames.join('", "')}" jako baza, z dodatkami odnoszącymi się do usług cukierniczych na wesela. Nazwy powinny sugerować różnorodność usług (np. torty ślubne, słodkie stoły, ciasta, makaroniki, dekoracje cukiernicze) i smak. Słowa kluczowe do wykorzystania: "${keywords.join('", "')}".
...
PRZYKŁADY NIEDOZWOLONYCH NAZW:
    ...
    -"Pyszności Ślubne"
    -"Torty i Słodkości"
    - "Cukiernia Ślubnych Smaków"
    - "Pracownia Weselnych Tortów"
    - "Słodkości Kreatywnej Elegancji"
    - "Piekarnia Czekoladowych Inspiracji"
  ...
  if (!companyName) {
    ...
    while (!companyName && fallbackAttempts < maxFallbackAttempts) {
      ...
      if (useThreeWords) {
        if (randomBase === "Cukiernia" || randomBase === "Piekarnia") {
          candidateName = `${randomBase} ${randomKeyword} ${randomAdditional}`; // np. "Cukiernia Ślubna Delikatność"
        } else {
          candidateName = `${randomBase} ${randomAdditional} ${randomKeyword}`; // np. "Pracownia Weselna Słodkość"
        }
      } else {
        if (randomBase === "Cukiernia" || randomBase === "Piekarnia") {
          candidateName = `${randomBase} ${randomKeyword}`; // np. "Cukiernia Tortów"
        } else {
          candidateName = `${randomBase} ${randomAdditional}`; // np. "Słodkości Ślubna"
        }
      }
    ...
    if (!companyName) {
      ...
      companyName = `Cukiernia ${uniqueSuffix}`; // np. "Cukiernia Słodkość-1234"
      usedCompanyNames.add(companyName);
      saveUsedCompanyNames(usedCompanyNames);
...
async function generateInitialText(categoryName: string, filterNames: string[], companyName: string): Promise<{ bandName: string; shortDescription: string; longDescription: string }> {
    ...
    2. Treść:
       ...
       - Opisz: historię firmy, specjalizację (torty ślubne, słodkie stoły, wypieki dekoracyjne), ofertę (ciasta, makaroniki, bezy, personalizowane torty), materiały (świeże owoce, czekolada, kremy), podejście do klienta (konsultacje smaków, dopasowanie do motywu)
       - Podkreśl unikalne wartości (smak, elegancja, kreatywność)
  ...
  async function expandTextPart1(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    4. Tematyka rozszerzenia:
       - Specyficzne usługi (np. torty piętrowe, mini desery, dekoracje jadalne)
       - Materiały i techniki (np. ręczne zdobienia, lukier królewski, świeże owoce)
       - Przykładowe realizacje (np. tort rustykalny, elegancki słodki stół)
  ...
  async function expandTextPart2(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    3. Wpleć motyw: "${keywords.join('", "')}"
    4. Tematyka rozszerzenia:
       - Doświadczenia z Parami Młodymi (np. konsultacje smaków, próby tortów)
       - Unikalne usługi (np. torty tematyczne, dostawa słodkości, mini wypieki)
       - Wizja i wartości (np. tworzenie smaku i piękna, radość gości)
       - Call-to-action (subtelnym tonem, np. zachęta do kontaktu w sprawie tortu)
...
async function generateOfferData(category: any): Promise<OfferData> {
  ...
  const allImages = [
    '/uploads/images/c1.png',
    '/uploads/images/c2.png',
    '/uploads/images/c3.png',
    '/uploads/images/c4.png',
    '/uploads/images/c5.png',
    '/uploads/images/c6.png',
    '/uploads/images/c7.png',
    '/uploads/images/c8.png',
    '/uploads/images/c9.png',
    '/uploads/images/c10.png',
    '/uploads/images/c11.png',
    '/uploads/images/c12.png',
    '/uploads/images/c13.png',
    '/uploads/images/c14.png',
    '/uploads/images/c15.png',
    '/uploads/images/c16.png',
    '/uploads/images/c17.png',
    '/uploads/images/c18.png',
    '/uploads/images/c19.png',
    '/uploads/images/c20.png',
    '/uploads/images/c21.png',
    '/uploads/images/c22.png',
    '/uploads/images/c23.png',
  ]
  ...
  media.splice(videoIndex, 0, {
    mediaType: 'video',
    mediaUrl: 'O475OP4mkwg?si=2PH-UJCQRRiXV-qf'
  });
...
async function generateReviewData(listingId: number): Promise<ReviewData[]> {
    ...
    const prompt = `
    Wygeneruj od 3 do 5 realistycznych opinii w języku polskim dla cukierni oferującej usługi weselne w kategorii "Cukiernia". 
    ...
    - "ratingQuality": ocena jakości (3-5, np. jakość wypieków, świeżość)
    - "ratingCommunication": ocena komunikacji (3-5)
    - "ratingCreativity": ocena kreatywności (3-5, np. dekoracje tortów, pomysły)
    - "ratingServiceAgreement": ocena zgodności z umową (3-5, np. terminowość)
    - "ratingAesthetics": ocena estetyki (3-5, np. wygląd tortów, słodkiego stołu)
        ...
        const defaultReviews = [
            {
              ...
              reviewText: "Tort ślubny był nie tylko pyszny, ale i pięknie udekorowany kremem i owocami. Słodki stół zachwycił wszystkich gości swoją elegancją!"
            },
            {
              ratingQuality: faker.number.int({ min: 4, max: 5 }),
              ...
              ratingAesthetics: faker.number.int({ min: 4, max: 5 }),
              reviewText: "Wypieki były delikatne i pełne smaku, a makaroniki dodały szyku naszemu weselu. Komunikacja mogłaby być szybsza, ale efekt końcowy był wart czekania."
            },
            {
              ...
              reviewText: "Tort był smaczny, choć krem mógłby być lżejszy. Dekoracja i słodki stół stworzyły magiczną atmosferę na naszym ślubie!"
            }
          ];
...
const category = categories.find((cat: any) => cat.service_category_id === 18);
