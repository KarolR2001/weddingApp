...
async function generateCompanyName(categoryName: string): Promise<string> {
    const keywords = ["Piękno", "Styl", "Uroda", "Harmonia", "Luksus", "Elegancja", "Relaks", "Glamour", "Szyk", "Naturalność", "Wyjątkowość", "Pasja", "Kreatywność", "Indywidualność", "Tradycja", "Nowoczesność", "Romantyzm", "Delikatność", "Dojazd", "Mobilność", "Usługi", "Fryzury", "Makijaż", "Manicure", "Pedicure", "Stylizacja", "Kreacje", "Fasony", "Dodatki", "Pan Młody", "Panna Młoda", "Doradztwo", "Fryzura", "Kreacja", "Dopasowanie", "Jakość", "Inspiracja", "Historia", "Doświadczenie", "Warkocz", "Damska"];
    const baseNames = ["Salon", "Studio", "Fryzjer", "Kosmetyczka", "Pracownia", "Gabinet", "Specjalista", "Mistrz"];
    const additionalWords = ["Fryzjerski", "Kosmetyczny", "Ślubny", "Mobilny"];
    ...
    const prompt = `
      ...
      2. Poprawna gramatyka i odmiana (np. "Salon Fryzjerski" a nie "Salon Fryzjer", "Studio Ślubne" a nie "Studio Ślub")
      3. Zakazane elementy:
         - Cyfry w jakiejkolwiek formie (np. "Studio 2", "Atelier Trzy")
         - Znaki specjalne poza myślnikami
         - Skróty i numery porządkowe (II, III, 1st)
      
      4. Preferowane są polskie nazwy, używające słów takich jak "${baseNames.join('", "')}" jako baza, z dodatkami odnoszącymi się do usług fryzjerskich i kosmetycznych (damskich i męskich) z dojazdem w dniu wesela. Nazwy powinny sugerować różnorodność usług (np. fryzury, makijaż, manicure, pedicure) lub mobilność. Słowa kluczowe do wykorzystania: "${keywords.join('", "')}".
      ...
      - "Salon Piękna Ślubnego"
      - "Studio Mobilne Urody"
      - "Fryzjer Elegancji"
      - "Kosmetyczka Kreatywności"
    ...
    if (!companyName) {
      ...
      while (!companyName && fallbackAttempts < maxFallbackAttempts) {
        ...
        if (useThreeWords) {
          ...
          if (randomBase === "Fryzjer" || randomBase === "Kosmetyczka" || randomBase === "Specjalista" || randomBase === "Mistrz") {
            candidateName = `${randomBase} ${randomKeyword} ${randomAdditional}`; // np. "Fryzjer Ślubny Styl"
          } else {
            candidateName = `${randomBase} ${randomAdditional} ${randomKeyword}`; // np. "Salon Mobilny Uroda"
          }
        } else {
          // Dwa słowa dla prostoty
          if (randomBase === "Fryzjer" || randomBase === "Kosmetyczka") {
            candidateName = `${randomBase} ${randomKeyword}`; // np. "Fryzjer Elegancji"
          } else {
            candidateName = `${randomBase} ${randomAdditional}`; // np. "Studio Ślubne"
          }
        }
      ...
      if (!companyName) {
        ...
        companyName = `Salon ${uniqueSuffix}`; // np. "Salon Piękno-1234"
        usedCompanyNames.add(companyName);
        saveUsedCompanyNames(usedCompanyNames);
...
async function generateInitialText(categoryName: string, filterNames: string[], companyName: string): Promise<{ bandName: string; shortDescription: string; longDescription: string }> {
    const keywords = ["Piękno", "Styl", "Uroda", "Harmonia", "Luksus", "Elegancja", "Relaks", "Glamour", "Szyk", "Naturalność", "Piękno", "Styl", "Uroda", "Harmonia", "Luksus", "Elegancja", "Relaks", "Glamour", "Szyk", "Naturalność", "Wyjątkowość", "Pasja", "Kreatywność", "Indywidualność", "Tradycja", "Nowoczesność", "Romantyzm", "Delikatność", "Dojazd", "Mobilność", "Usługi", "Fryzury", "Makijaż", "Manicure", "Pedicure", "Stylizacja", "Kreacje", "Fasony", "Dodatki", "Pan Młody", "Panna Młoda", "Doradztwo", "Irokez", "Fryzura", "Makijaż", "Kreacja", "Stylizacja", "Dopasowanie", "Jakość", "Materiał", "Inspiracja", "Historia", "Doświadczenie", "Pasja", "Kreatywność", "Warkocz","Damska"];
    const randomKeyword = keywords[Math.floor(Math.random() * keywords.length)];
    const prompt = `
    ...
    2. Treść:
       ...
       - Opisz: historię salonu, specjalizację (fryzury damskie i męskie, makijaż ślubny, manicure, pedicure), ofertę z dojazdem do klienta w dniu wesela, różnorodność usług (np. stylizacja brody, upięcia, pielęgnacja dłoni i stóp), użyte produkty (marki, jakość)
       - Podkreśl unikalne wartości (doświadczenie, mobilność, indywidualne podejście, dbałość o detal)
  ...
  async function expandTextPart1(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    const keywords = ["Elegancja", "Styl", "Klasyka", "Moda", "Szyk", "Prestiż", "Harmonia", "Nowoczesność", "Gentleman", "Fason", "Garnitur", "Dodatki", "Pan Młody", "Kreacja", "Wyjątkowość", "Indywidualność", "Tradycja", "Nowoczesność", "Stylizacja", "Marynarka", "Spodnie", "Krawat", "Muchy", "Buty", "Akcesoria", "Dopasowanie", "Jakość", "Materiał", "Inspiracja", "Historia", "Doświadczenie", "Pasja", "Kreatywność", "Elegancja", "Romantyzm", "Minimalizm", "Luksus", "Delikatność", "Moderne", "Brąz", "Czarny", "Granatowy", "Szary", "Biały", "Beżowy", "Kremowy", "Zielony", "Niebieski", "Czerwony", "Różowy", "Fioletowy", "Złoty", "Srebrny", ];
    const randomKeyword = keywords[Math.floor(Math.random() * keywords.length)];
    const prompt = `
    ...
    4. Tematyka rozszerzenia:
       - Specyficzne usługi (np. upięcia ślubne, makijaż naturalny lub wieczorowy, manicure hybrydowy, pedicure, strzyżenie męskie, stylizacja brody)
       - Produkty i techniki (np. profesjonalne kosmetyki, trwałe lakiery, pielęgnacja skóry)
       - Przykładowe realizacje (np. przygotowanie Panny Młodej i Pana Młodego w dniu ślubu)
  ...
  async function expandTextPart2(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    3. Wpleć motyw: "${randomKeyword}"
    4. Tematyka rozszerzenia:
       - Doświadczenia z klientami w dniu wesela (np. dojazd, punktualność)
       - Unikalne usługi (np. pakiet ślubny dla Pary Młodej, ekspresowa stylizacja gości)
       - Wizja i wartości (np. podkreślanie naturalnego piękna, komfort klienta)
       - Call-to-action (subtelnym tonem, np. zachęta do kontaktu przed ślubem)
...
async function generateOfferData(category: any): Promise<OfferData> {
  ...
  const allImages = [
    '/uploads/images/h1.png',
    '/uploads/images/h2.png',
    '/uploads/images/h3.png',
    '/uploads/images/h4.png',
    '/uploads/images/h5.png',
    '/uploads/images/h6.png',
    '/uploads/images/h7.png',
    '/uploads/images/h8.png',
    '/uploads/images/h9.png',
    '/uploads/images/h10.png',
    '/uploads/images/h11.png',
    '/uploads/images/h12.png',
    '/uploads/images/h13.png',
    '/uploads/images/h14.png',
    '/uploads/images/h15.png',
    '/uploads/images/h16.png',
    '/uploads/images/h17.png',
    '/uploads/images/h18.png',
    '/uploads/images/h19.png',
    '/uploads/images/h20.png',
    '/uploads/images/h21.png',
    '/uploads/images/h22.png',
    '/uploads/images/h23.png'
  ]
  ...
  media.splice(videoIndex, 0, {
    mediaType: 'video',
    mediaUrl: '3Hz6Kf1qL84?si=KIifUbrmPNZZPA2t'
  });
...
async function generateReviewData(listingId: number): Promise<ReviewData[]> {
    const prompt = `
    Wygeneruj od 3 do 5 realistycznych opinii w języku polskim dla salonu fryzjerskiego i kosmetycznego oferującego usługi z dojazdem w dniu wesela. 
    ...
    - "ratingQuality": ocena jakości (3-5, np. jakość fryzury, makijażu, manicure)
    - "ratingCommunication": ocena komunikacji (3-5)
    - "ratingCreativity": ocena kreatywności (3-5, np. stylizacja, dobór kolorów)
    - "ratingServiceAgreement": ocena zgodności z umową (3-5, np. punktualność dojazdu)
    - "ratingAesthetics": ocena estetyki (3-5, np. efekt końcowy, harmonia wyglądu)
        ...
        const defaultReviews = [
            {
              ...
              reviewText: "Fryzura i makijaż były perfekcyjne, a dojazd w dniu ślubu punktualny. Manicure hybrydowy trzymał się przez całe wesele!"
            },
            {
              ratingQuality: faker.number.int({ min: 4, max: 5 }),
              ...
              ratingAesthetics: faker.number.int({ min: 4, max: 5 }),
              reviewText: "Stylizacja brody i fryzura męska wyszły świetnie, pedicure też na poziomie. Komunikacja przed ślubem mogłaby być szybsza."
            },
            {
              ...
              reviewText: "Makijaż i upięcie były piękne, choć lakier na paznokciach mógłby być trwalszy. Ekipa przyjechała na czas i stworzyła cudowną atmosferę!"
            }
          ];
...
const category = categories.find((cat: any) => cat.service_category_id === 17);
...
testMultipleOffers(5).catch(error => {
