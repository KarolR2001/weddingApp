...
async function generateCompanyName(categoryName: string): Promise<string> {
    const keywords = ["Elegancja", "Urok", "Tradycja", "Splendor", "Natura", "Harmonia", "Luksus", "Kameralność", "Historia", "Nowoczesność"];
    const baseNames = ["Dworek", "Sala Balowa", "Restauracja"]; // Zaktualizowana lista bazowa
    const additionalWords = ["Weselny", "Bankietowy", "Ślubny", "Gościnny"];
    ...
    const prompt = `
      ...
      2. Poprawna gramatyka i odmiana (np. "Dworek Weselny" a nie "Dworek Wesele")
      3. Zakazane elementy:
         - Cyfry w jakiejkolwiek formie (np. "Sala 2", "Dworek Trzy")
      ...
      5. Preferowane są polskie nazwy, używające słów takich jak "Dworek", "Sala Balowa", "Restauracja" jako baza, z dodatkami odnoszącymi się do wesel.
...
async function generateInitialText(categoryName: string, filterNames: string[], companyName: string): Promise<{ bandName: string; shortDescription: string; longDescription: string }> {
    ...
    2. Treść:
       ...
       - Opisz: historię firmy, specjalizację, ofertę (np. sale na różne liczby gości, dekoracje), udogodnienia, otoczenie
       - Podkreśl unikalne wartości (lokalizacja, atmosfera, jakość obsługi)
  ...
  async function expandTextPart1(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    4. Tematyka rozszerzenia:
       - Specyficzne cechy sal (np. wielkość, styl aranżacji, parkiet taneczny)
       - Udogodnienia (np. ogród, parking, noclegi)
       - Przykładowe aranżacje lub wydarzenia
  ...
  async function expandTextPart2(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    3. Wpleć motyw: "${randomKeyword}"
    4. Tematyka rozszerzenia:
       - Doświadczenia z parami młodymi
       - Unikalne usługi (np. koordynacja wesela, catering)
...
async function generateOfferData(category: any): Promise<OfferData> {
  ...
  const allImages = [
    '/uploads/images/100.png',
    '/uploads/images/101.png',
    '/uploads/images/102.png',
    '/uploads/images/103.png',
    '/uploads/images/104.png',
    '/uploads/images/105.png',
    '/uploads/images/106.png',
    '/uploads/images/107.png',
    '/uploads/images/108.png',
    '/uploads/images/110.png',
    '/uploads/images/111.png',
    '/uploads/images/112.png',
    '/uploads/images/113.png',
    '/uploads/images/114.png',
    '/uploads/images/115.png',
    '/uploads/images/116.png',
    '/uploads/images/117.png',
    '/uploads/images/118.png',
    '/uploads/images/119.png',
    '/uploads/images/120.png',
    '/uploads/images/121.png',
    '/uploads/images/122.png'
  ];
  ...
  media.splice(videoIndex, 0, {
    mediaType: 'video',
    mediaUrl: 'AjoAIo2o_uU?si=L87C-f2vtVmDNHOX'
  });
...
async function generateReviewData(listingId: number): Promise<ReviewData[]> {
    const prompt = `
      Wygeneruj od 3 do 5 realistycznych opinii w języku polskim dla sali weselnej w branży ślubnej. 
      ...
      - "ratingQuality": ocena jakości (3-5, np. jakość sali, jedzenia)
      - "ratingCommunication": ocena komunikacji (3-5)
      - "ratingCreativity": ocena kreatywności (3-5, np. dekoracje, aranżacje)
      - "ratingServiceAgreement": ocena zgodności z umową (3-5)
      - "ratingAesthetics": ocena estetyki (3-5, np. wygląd sali, otoczenie)
      ...
      const defaultReviews = [
        {
          ...
          reviewText: "Sala weselna była przepiękna, jedzenie smaczne, a obsługa bardzo pomocna. Wszystko przebiegło zgodnie z ustaleniami!"
        },
        {
          ratingQuality: faker.number.int({ min: 4, max: 5 }),
          ...
          ratingAesthetics: faker.number.int({ min: 4, max: 5 }),
          reviewText: "Wystrój sali zachwycał, a organizacja wesela była na najwyższym poziomie. Komunikacja czasem trwała dłużej, ale efekt końcowy wart czekania."
        },
        {
          ...
          reviewText: "Sala wyglądała cudownie, a otoczenie dodało uroku naszemu weselu. Jedzenie mogłoby być nieco lepsze, ale obsługa stanęła na wysokości zadania."
        },
        {
          ratingQuality: faker.number.int({ min: 4, max: 5 }),
          ratingCommunication: faker.number.int({ min: 4, max: 5 }),
          ...
          ratingServiceAgreement: faker.number.int({ min: 4, max: 5 }),
          ratingAesthetics: faker.number.int({ min: 5, max: 5 }),
          reviewText: "Miejsce idealne na wesele, z pięknym ogrodem i elegancką salą. Dekoracje były dość standardowe, ale całość i tak zrobiła wrażenie."
        }
      ];
...
const category = categories.find((cat: any) => cat.service_category_id === 12);
