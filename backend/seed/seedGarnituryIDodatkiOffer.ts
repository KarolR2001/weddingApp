...
async function generateCompanyName(categoryName: string): Promise<string> {
    const keywords = ["Elegancja", "Styl", "Klasyka", "Moda", "Szyk", "Prestiż", "Harmonia", "Nowoczesność", "Gentleman", "Fason"];
    const baseNames = ["Salon", "Atelier", "Butik"];
    const additionalWords = ["Garniturów", "Ślubny", "Męski", "Modowy"];
    
    const prompt = `
    Tworzę bazę ofert dla firm lub ofert indywidualnych w branży ślubnej, specjalizujących się w kategorii "${categoryName}". 
    Potrzebuję unikalnych, chwytliwych nazw firm lub zestawów ubrań spełniających następujące wymagania:
    
    ŚCISŁE ZASADY:
    1. Format: 2-4 słowa, bez cyfr (arabskich, rzymskich ani słownie)
    2. Poprawna gramatyka i odmiana (np. "Salon Garniturów" a nie "Salon Garnitur", "Atelier Ślubne" a nie "Atelier Ślub")
    3. Zakazane elementy:
       - Cyfry w jakiejkolwiek formie (np. "Butik 2", "Styl Trzy")
    ...
    5. Preferowane są polskie nazwy, używające słów takich jak "Salon", "Atelier", "Butik" jako baza, z dodatkami odnoszącymi się do garniturów i dodatków ślubnych dla Pana Młodego. Mogą to być nazwy salonów (np. "Salon Elegancji") lub pojedynczych zestawów ubrań (np. "Styl Gentelman").
    ...
    - "Salon Garniturów Ślubnych"
    - "Atelier Prestiżu"
    - "Butik Męski Styl"
    - "Elegancja Gentelman" (nazwa zestawu)
...
async function generateInitialText(categoryName: string, filterNames: string[], companyName: string): Promise<{ bandName: string; shortDescription: string; longDescription: string }> {
    ...
    const prompt = `
    Jesteś copywriterem specjalizującym się w tekstach dla branży ślubnej. Stwórz kompleksowy opis w języku polskim dla firmy lub zestawu "${companyName}" (kategoria: ${categoryName}).
    ...
    1. Struktura:
       ...
       - LongDescription: pełny tekst "O nas" lub opisu oferty (ok. 20 zdań) stanowiący SAMODZIELNĄ CAŁOŚĆ
    
    2. Treść:
       - Uwzględnij filtry: ${filterNames.join(', ')}
       - Wpleć motyw: "${randomKeyword}"
       - Opisz: historię salonu lub inspirację zestawu, specjalizację (np. garnitury ślubne, dodatki), ofertę (marynarki, spodnie, krawaty, buty, muchy), jakość materiałów, proces doboru
       - Podkreśl unikalne wartości (styl, dopasowanie, doświadczenie)
  ...
  async function expandTextPart1(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    const randomKeyword = keywords[Math.floor(Math.random() * keywords.length)];
    const prompt = `
    Jesteś tym samym copywriterem co wcześniej. Rozwiń istniejący opis firmy lub zestawu "${bandName}" o 5 NOWYCH, SPÓJNYCH zdań.
    ...
    4. Tematyka rozszerzenia:
       - Specyficzne elementy oferty (np. kroje garniturów: slim, classic; dodatki: krawaty, muchy, buty)
       - Materiały (np. wełna, bawełna, skóra)
       - Przykładowe stylizacje lub realizacje
  ...
  async function expandTextPart2(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    3. Wpleć motyw: "${randomKeyword}"
    4. Tematyka rozszerzenia:
       - Doświadczenia z Panami Młodymi
       - Unikalne usługi (np. szycie na miarę, doradztwo stylistyczne)
       - Wizja i wartości (np. elegancja na miarę ślubu)
...
async function generateOfferData(category: any): Promise<OfferData> {
  ...
  const allImages = [
    '/uploads/images/suit1.png',
    '/uploads/images/suit2.png',
    '/uploads/images/suit3.png',
    '/uploads/images/suit4.png',
    '/uploads/images/suit5.png',
    '/uploads/images/suit6.png',
    '/uploads/images/suit7.png',
    '/uploads/images/suit8.png',
    '/uploads/images/suit9.png',
    '/uploads/images/suit10.png',
    '/uploads/images/suit11.png',
    '/uploads/images/suit12.png',
    '/uploads/images/suit13.png',
    '/uploads/images/suit14.png',
    '/uploads/images/suit15.png',
    '/uploads/images/suit16.png',
    '/uploads/images/suit17.png',
    '/uploads/images/suit18.png',
    '/uploads/images/suit19.png',
    '/uploads/images/suit20.png',
    '/uploads/images/suit21.png',
    '/uploads/images/suit22.png',
    '/uploads/images/suit23.png',
    '/uploads/images/suit24.png',
    '/uploads/images/suit25.png',
  ];
  ...
  media.splice(videoIndex, 0, {
    mediaType: 'video',
    mediaUrl: 'HxKVPFF2BEY?si=b6wRWntRFkrPyn0x'
  });
  ...
  const offerData: OfferData = {
    ...
    longDescription: fullLongDescription,
    priceMin: faker.number.int({ min: 700, max: 2000 }),
    priceMax: faker.number.int({ min: 1200, max: 10000 }),
...
async function generateReviewData(listingId: number): Promise<ReviewData[]> {
    const prompt = `
    Wygeneruj od 3 do 5 realistycznych opinii w języku polskim dla salonu garniturów lub zestawu ubrań ślubnych dla Pana Młodego. 
    ...
    - "ratingQuality": ocena jakości (3-5, np. jakość garnituru, materiałów)
    - "ratingCommunication": ocena komunikacji (3-5)
    - "ratingCreativity": ocena kreatywności (3-5, np. dobór dodatków, stylizacja)
    - "ratingServiceAgreement": ocena zgodności z umową (3-5)
    - "ratingAesthetics": ocena estetyki (3-5, np. wygląd, dopasowanie)
        ...
        const defaultReviews = [
            {
              ...
              reviewText: "Garnitur był świetnie skrojony, a krawat i buty idealnie dobrane. Obsługa pomogła mi stworzyć elegancką stylizację na ślub!"
            },
            {
              ratingQuality: faker.number.int({ min: 4, max: 5 }),
              ...
              ratingAesthetics: faker.number.int({ min: 4, max: 5 }),
              reviewText: "Marynarka i spodnie były wysokiej jakości, a mucha dodała uroku. Komunikacja z salonem mogłaby być szybsza, ale efekt końcowy super."
            },
            {
              ...
              reviewText: "Garnitur wyglądał świetnie, choć materiał mógłby być bardziej komfortowy. Doradztwo w doborze dodatków na najwyższym poziomie!"
            }
          ];
...
const category = categories.find((cat: any) => cat.service_category_id === 16);
...
testMultipleOffers(4).catch(error => {
