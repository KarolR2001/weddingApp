...
async function generateCompanyName(categoryName: string): Promise<string> {
    const keywords = ["Elegancja", "Koronka", "Jedwab", "Boho", "Klasyka", "Romantyzm", "Minimalizm", "Luksus", "Delikatność", "Moderne"];
    const baseNames = ["Salon", "Atelier", "Suknie", "Moda"];
    const additionalWords = ["Ślubne", "Na Miarę", "Couture", "Bridal"];
    ...
    2. Poprawna gramatyka i odmiana (np. "Salon Sukien Ślubnych" a nie "Salon Suknia Ślubne")
    3. Zakazane elementy:
       - Cyfry w jakiejkolwiek formie (np. "Atelier 2", "Kolekcja Trzy")
    ...
    - "Atelier Królewskiej Elegancji II"
    - "Salon Sukien 3"
    - "Moda Ślubna 2024"
    - "Pracownia Pierwsza"
...
`;
    ...
    // Poprawiony wzorzec do wykrywania cyfr - uwzględniający różne warianty
    ...
    while (!companyName && attempts < maxAttempts) {
        ...
        try {
            ...
            // Dokładne sprawdzenie czy nazwa zawiera cyfry lub słowne liczby
    ...
    if (!companyName) {
        ...
        while (!companyName && fallbackAttempts < maxFallbackAttempts) {
            ...
            // Losowanie różnych kombinacji słów dla większej różnorodności
            ...
            // Ostateczne sprawdzenie nazwy awaryjnej
        ...
        if (companyName) {
            ...
            // Ostateczne awaryjne rozwiązanie, jeśli wszystko zawiedzie
            companyName = `Salon Ślubny ${Date.now().toString().slice(-4)}`;
            usedCompanyNames.add(companyName);
            saveUsedCompanyNames(usedCompanyNames);
...
async function generateInitialText(categoryName: string, filterNames: string[], companyName: string): Promise<{ bandName: string; shortDescription: string; longDescription: string }> {
    ...
    const prompt = `
      ...
      2. Treść:
         ...
         - Opisz: historię firmy, specjalizację, ofertę (suknie gotowe/na miarę), proces współpracy, materiały
         - Podkreśl unikalne wartości (doświadczenie, podejście do klienta, jakość)
  ...
  async function expandTextPart1(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    const prompt = `
      ...
      4. Tematyka rozszerzenia:
         - Specyficzne rodzaje sukien (np. królowe, a-line, mermaid)
         - Techniki krawieckie i materiały
         - Przykładowe realizacje/projekty
  ...
  async function expandTextPart2(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    const prompt = `
      ...
      4. Tematyka rozszerzenia:
         ...
         - Unikalne usługi (np. przymiarki prywatne)
         - Wizja i wartości firmy
...
async function generateOfferData(category: any): Promise<OfferData> {
  ...
  const allImages = [
    '/uploads/images/900.png',
    '/uploads/images/901.png',
    '/uploads/images/902.png',
    '/uploads/images/903.png',
    '/uploads/images/904.png',
    '/uploads/images/905.png',
    '/uploads/images/906.png',
    '/uploads/images/907.png',
    '/uploads/images/908.png',
    '/uploads/images/910.png',
    '/uploads/images/911.png',
    '/uploads/images/16.png',
    '/uploads/images/15.png'
  ];
  ...
  media.splice(videoIndex, 0, {
    mediaType: 'video',
    mediaUrl: 'UDc7smGhD68?si=r-nDQaD8SHz7ihZM'
  });
  ...
  const offerData: OfferData = {
    ...
    longDescription: fullLongDescription,
    priceMin: faker.number.int({ min: 1000, max: 3000 }),
    priceMax: faker.number.int({ min: 3500, max: 10000 }),
  ...
  if (!offersNationwideService) {
    offerData.rangeInKm = faker.number.int({ min: 50, max: 200 });
  }
...
async function generateReviewData(listingId: number): Promise<ReviewData[]> {
    const prompt = `
      Wygeneruj od 3 do 5 realistycznych opinii w języku polskim dla salonu ślubnego lub atelier w branży ślubnej (np. suknie ślubne, szycie na miarę). 
      ...
      - "ratingQuality": ocena jakości (3-5, np. jakość sukni)
      - "ratingCommunication": ocena komunikacji (3-5)
      - "ratingCreativity": ocena kreatywności (3-5, np. projekt sukni)
      - "ratingServiceAgreement": ocena zgodności z umową (3-5)
      - "ratingAesthetics": ocena estetyki (3-5, np. wygląd sukni, atmosfera w salonie)
    ...
    while (attempts < maxAttempts && reviews.length < 3) {
      try {
        ...
        //console.log('Surowa odpowiedź modelu:', response.response);
        let parsedResponse;
        try {
          parsedResponse = JSON.parse(response.response);
        } catch (error) {
          //console.error('Błąd parsowania JSON-a w generateReviewData:', error, 'Surowa odpowiedź:', response.response);
          attempts++;
          continue;
        }
  
        // Jeśli odpowiedź jest obiektem z polem "opinie", wyciągnij tablicę
        if (!Array.isArray(parsedResponse) && parsedResponse.opinie && Array.isArray(parsedResponse.opinie)) {
         // console.warn('Odpowiedź zawiera pole "opinie", wyciągam tablicę:', parsedResponse);
          parsedResponse = parsedResponse.opinie;
        } else if (!Array.isArray(parsedResponse)) {
//          console.warn('Odpowiedź nie jest tablicą, konwertuję na tablicę:', parsedResponse);
        ...
        if (validReviews.length > 0) {
          ...
          console.warn('Brak ważnych opinii w odpowiedzi:', parsedResponse);
        }
    ...
    if (reviews.length < 3) {
      console.log('Nie udało się wygenerować wystarczającej liczby opinii, używam domyślnych.');
      const defaultReviews = [
        {
          ...
          reviewText: "Suknia uszyta na miarę była przepiękna, idealnie dopasowana. Obsługa bardzo miła i profesjonalna!"
        },
        {
          ratingQuality: faker.number.int({ min: 4, max: 5 }),
          ...
          ratingAesthetics: faker.number.int({ min: 4, max: 5 }),
          reviewText: "Wybrałam gotową suknię i jestem zachwycona jej jakością. Komunikacja mogłaby być szybsza, ale efekt końcowy super."
        },
        {
          ...
          reviewText: "Suknia wyglądała pięknie, choć materiał mógłby być lepszy. Świetna obsługa i kreatywny projekt!"
        }
      ];
...
const category = categories.find((cat: any) => cat.service_category_id === 7);
...
testMultipleOffers(2).catch(error => {
