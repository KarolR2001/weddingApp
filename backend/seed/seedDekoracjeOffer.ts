...
async function generateCompanyName(categoryName: string): Promise<string> {
    const keywords = ["Dekoracja", "Ozdoba", "Aranżacja", "Styl", "Elegancja", "Piękno", "Harmonia", "Kreatywność", "Inspiracja", "Detal", "Światło", "Kolory", "Tkanina", "Kwiaty", "Ślub", "Wesele", "Sala", "Plener", "Romantyzm", "Nowoczesność", "Tradycja", "Atmosfera", "Magia", "Urok", "Design", "Szyk", "Girlandy", "Balony", "Świece", "Nastroje", "Wystrój", "Kompozycja"];
    const baseNames = ["Studio", "Pracownia", "Dekorator", "Projektant"];
    const additionalWords = ["Ślubne", "Weselne", "Kreatywne", "Eleganckie"];
    ...
    const prompt = `
      ...
      2. Poprawna gramatyka i odmiana (np. "Studio Ślubne" a nie "Studio Ślub", "Pracownia Dekoracji" a nie "Pracownia Dekoracja")
      3. Zakazane elementy:
         - Cyfry w jakiejkolwiek formie (np. "Studio 2", "Dekorator Trzy")
         - Znaki specjalne poza myślnikami
         - Skróty i numery porządkowe (II, III, 1st)
      4. Preferowane są polskie nazwy, używające słów takich jak "${baseNames.join('", "')}" jako baza, z dodatkami odnoszącymi się do usług dekoratorskich na wesela. Nazwy powinny sugerować różnorodność usług (np. dekoracje sal weselnych, aranżacje stołów, ozdoby plenerowe, dekoracje dla Pary Młodej, oświetlenie) i styl. Słowa kluczowe do wykorzystania: "${keywords.join('", "')}".
      ...
      - "Studio Ślubnych Inspiracji"
      - "Pracownia Weselnej Magii"
      - "Dekorator Kreatywnych Detali"
      - "Projektant Eleganckich Aranżacji"
    ...
    if (!companyName) {
      ...
      while (!companyName && fallbackAttempts < maxFallbackAttempts) {
        ...
        if (useThreeWords) {
          if (randomBase === "Dekorator") {
            candidateName = `${randomBase} ${randomKeyword} ${randomAdditional}`; // np. "Dekorator Ślubnych Inspiracji"
          } else {
            candidateName = `${randomBase} ${randomAdditional} ${randomKeyword}`; // np. "Studio Weselne Dekoracje"
          }
        } else {
          if (randomBase === "Dekorator") {
            candidateName = `${randomBase} ${randomKeyword}`; // np. "Dekorator Elegancji"
          } else {
      ...
      if (!companyName) {
        ...
        companyName = `Studio ${uniqueSuffix}`; // np. "Studio Dekoracja-1234"
        usedCompanyNames.add(companyName);
        saveUsedCompanyNames(usedCompanyNames);
...
async function generateInitialText(categoryName: string, filterNames: string[], companyName: string): Promise<{ bandName: string; shortDescription: string; longDescription: string }> {
    ...
    2. Treść:
       ...
       - Opisz: historię firmy, specjalizację (dekoracje sal weselnych, aranżacje stołów, ozdoby plenerowe), ofertę (girlandy, świece, balony, dekoracje dla Pary Młodej, oświetlenie dekoracyjne), materiały (tkaniny, kwiaty, dodatki), podejście do klienta (konsultacje, personalizacja)
       - Podkreśl unikalne wartości (kreatywność, elegancja, dbałość o atmosferę)
  ...
  async function expandTextPart1(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    4. Tematyka rozszerzenia:
       - Specyficzne usługi (np. dekoracje łuków ślubnych, girlandy świetlne, ozdoby stołów, dekoracje krzeseł)
       - Materiały i techniki (np. tkaniny dekoracyjne, balony z helem, ręczne aranżacje)
       - Przykładowe realizacje (np. wesele rustykalne, elegancka sala bankietowa)
  ...
  async function expandTextPart2(existingText: string, bandName: string, filterNames: string[]): Promise<string> {
    ...
    3. Wpleć motyw: "${keywords.join('", "')}"
    4. Tematyka rozszerzenia:
       - Doświadczenia z Parami Młodymi (np. konsultacje wizji, dopasowanie do motywu wesela)
       - Unikalne usługi (np. dekoracje tematyczne, wypożyczanie rekwizytów, szybki montaż)
       - Wizja i wartości (np. tworzenie magicznej atmosfery, dbałość o detale)
       - Call-to-action (subtelnym tonem, np. zachęta do kontaktu w sprawie aranżacji)
...
async function generateOfferData(category: any): Promise<OfferData> {
  ...
  const allImages = [
    ...
    '/uploads/images/flor13.png',
    '/uploads/images/flor12.png',
    '/uploads/images/flor11.png',
    '/uploads/images/flor8.png',
    '/uploads/images/1731596984053-anuwa5202_a_chic_wedding_scene_with_an_ivory_and_rose_gold_colo_82f4af9e-7af7-4f93-9678-2f3c53f7ebd6.png',
    '/uploads/images/1731596761483-anuwa5202_a_chic_wedding_scene_with_an_ivory_and_rose_gold_colo_2d482737-73a5-456c-850d-de6c4d609374.png',
    '/uploads/images/1731596605337-corapearl_a_suite_of_wedding_invites_with_hand_drawn_painted_il_a811a9b7-0035-4387-8425-c80f7c53afce.png',
    '/uploads/images/115.png',
    '/uploads/images/109.png',
    '/uploads/images/10.png',
    '/uploads/images/dec1.png',
    '/uploads/images/dec2.png',
    '/uploads/images/dec3.png',
    '/uploads/images/dec4.png',
    '/uploads/images/dec5.png',
    '/uploads/images/dec6.png',
    '/uploads/images/dec7.png'
  ]
  ...
  media.splice(videoIndex, 0, {
    mediaType: 'video',
    mediaUrl: 'CkmrkIDEQTE?si=-AvvTmUjs_A4rWrJ'
  });
  ...
  const offerData: OfferData = {
    ...
    longDescription: fullLongDescription,
    priceMin: faker.number.int({ min: 500, max: 800 }),
    priceMax: faker.number.int({ min: 2000, max: 4000 }),
...
async function generateReviewData(listingId: number): Promise<ReviewData[]> {
    ...
    const prompt = `
    Wygeneruj od 3 do 5 realistycznych opinii w języku polskim dla firmy lub osoby oferującej usługi dekoratorskie w kategorii "Dekoracje". 
    ...
    - "ratingQuality": ocena jakości (3-5, np. jakość materiałów, trwałość dekoracji)
    - "ratingCommunication": ocena komunikacji (3-5)
    - "ratingCreativity": ocena kreatywności (3-5, np. aranżacje, pomysły)
    - "ratingServiceAgreement": ocena zgodności z umową (3-5, np. terminowość)
    - "ratingAesthetics": ocena estetyki (3-5, np. wygląd dekoracji, harmonia)
        ...
        const defaultReviews = [
            {
              ...
              reviewText: "Dekoracje sali weselnej zachwyciły elegancją i stylem, a girlandy świetlne dodały magii. Wszystko zostało perfekcyjnie dopasowane do naszej wizji!"
            },
            {
              ratingQuality: faker.number.int({ min: 4, max: 5 }),
              ...
              ratingAesthetics: faker.number.int({ min: 4, max: 5 }),
              reviewText: "Aranżacja stołów i pleneru była kreatywna i pełna uroku, choć komunikacja mogła być szybsza. Efekt końcowy stworzył niezapomnianą atmosferę."
            },
            {
              ...
              reviewText: "Dekoracje były piękne, choć niektóre balony mogłyby być trwalsze. Kompozycja świetlna i wystrój sali zachwyciły wszystkich gości!"
            }
          ];
...
const category = categories.find((cat: any) => cat.service_category_id === 5);
