...
export const printReportPDF = (report: any, logoUrl: string) => {
  const doc = new jsPDF();
  ...
  doc.setFontSize(16);
  doc.text(`Raport: ${report.reportName}`, pageWidth / 2, margin + 10, {
    align: "center",
  });
  ...
  // Generowanie PDF jako Blob
  const pdfBlob = doc.output("blob");

  // Utworzenie URL dla Blob
  const pdfURL = URL.createObjectURL(pdfBlob);

  // Tworzenie ukrytego iframe
  ...
  iframe.style.zIndex = "10000"; // Wyższy z-index, aby było nad całą zawartością
  iframe.style.backgroundColor = "white"; // Opcjonalne, dla estetyki
  iframe.style.border = "none";
  iframe.src = pdfURL;

  // Po załadowaniu wymuszenie drukowania
  iframe.onload = () => {
    iframe.contentWindow?.print();
  };

  // Dodaj iframe do dokumentu
  document.body.appendChild(iframe);

  // Usuń iframe po 10 sekundach (czas na zakończenie drukowania)
  setTimeout(() => {
