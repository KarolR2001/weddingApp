...
const CoupleRegisterPage: React.FC = () => {
  ...
  const handleNameChange = (setter: React.Dispatch<React.SetStateAction<string>>, field: keyof ValidationState, name: string) => {
    const filteredName = name.replace(/[^a-zA-Z\s]/g, '');
    setter(filteredName);
    setValidation((prevValidation) => ({
      ...prevValidation,
      [field]: filteredName.trim() !== '',
    }));
  };
  ...
  const validateAllFields = () => {
    const isBrideNameValid = brideName.trim() !== '';
    const isGroomNameValid = groomName.trim() !== '';
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Uruchamiamy `polling` po rejestracji
      const intervalId = setInterval(async () => {
        try {
          const { data } = await axios.get(`http://localhost:5000/auth/check-verification?email=${email}`);
          if (data.isVerified) {
            clearInterval(intervalId);
            setIsVerified(true); // Ustawienie stanu `isVerified` na true
            setIsLoading(false); // Wyłączenie spinnera
            navigate('/login');
          }
        } catch (error) {
          console.log('Błąd podczas sprawdzania statusu weryfikacji.');
        }
      }, 5000); // Interwał co 5 sekund
    } catch (error) {
            ...
            <div className={styles.nameContainer}>
              <div className={styles.inputWrapper}>
                <Input2
                  ...
                  type="text"
                  value={brideName}
                  onChange={(e) => handleNameChange(setBrideName, 'brideName', e.target.value)}
                  isValid={validation.brideName}
                />
              </div>
              <div className={styles.inputWrapper}>
                <Input2
                  ...
                  type="text"
                  value={groomName}
                  onChange={(e) => handleNameChange(setGroomName, 'groomName', e.target.value)}
                  isValid={validation.groomName}
                />
              </div>
            </div>
